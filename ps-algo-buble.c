...
/*   Updated: 2025/02/01 15:13:39 by abessa-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (1)
	{
		if (ps->verbose)
		{
			ft_printf("Buble sort: comparing postions 1 (%i) and 2 (%i)\n",
				get_value_from_pos(ps, chr, 1), get_value_from_pos(ps, chr, 2));
		}
		if ((get_value_from_pos(ps, chr, 1) > get_value_from_pos(ps, chr, 2))
			&& (get_value_from_pos(ps, chr, 1) != max(ps, chr)))
			swap_stack(ps, chr);
		else
			rotate_stack(ps, chr);
